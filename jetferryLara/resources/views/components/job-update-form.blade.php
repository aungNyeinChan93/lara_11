...
<div class="mx-auto max-w-screen-xl px-4 py-16 sm:px-6 lg:px-8">
    <div class="mx-auto max-w-lg">
        <h1 class="text-center text-2xl font-bold text-indigo-600 sm:text-3xl">Update Job</h1>
        ...
        <form action="{{ route('jobPosition.update',$job->id) }}" method="POST"
            ...
            <div class="mt-4">
                ...
                <div class="relative">
                    <input type="text" class="w-full rounded-lg border-gray-200 p-4 pe-12 text-sm shadow-sm"
                        name="title" value="{{ old('title',$job->title) }}" placeholder="Enter title" />
                    @error('title')
            ...
            <div class="mt-4">
                ...
                <div class="relative">
                    <input type="text" class="w-full rounded-lg border-gray-200 p-4 pe-12 text-sm shadow-sm"
                        name="salary" value="{{ old('salary',$job->salary) }}" placeholder="Enter salary" />
                    @error('salary')
                        <small class="text-red-600 text-md">{{ $message }}</small>
                    @enderror
                </div>
            ...
            <div class="mt-4">
                ...
                <select id="employer_id" name="employer_id" class="w-full rounded-lg border-gray-200 p-4 pe-12 text-sm shadow-sm">
                    ...
                    @foreach ($employers as $employer)
                        <option value="{{ $employer->id }}" @if ($job->employer_id == $employer->id ) selected @endif>
                            {{ $employer->name }}
                        </option>
                    @endforeach
                </select>
